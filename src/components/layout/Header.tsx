...
export const Header: React.FC = () => {
  ...
  const { isMobileMenuOpen, toggleMobileMenu, openCart, openWishlist } = useUIStore()

  const [showPerformanceDashboard, setShowPerformanceDashboard] = useState(false)

  // Hydration-safe cart count (optional; Zustand may already be fine)
  const [cartItemCount, setCartItemCount] = useState<number>(0)
  useState(() => setCartItemCount(getItemCount()))
      ...
      >
        <div className="container mx-auto px-4">
          <div className="flex h-16 items-center justify-between">
            {/* Logo */}
            <motion.div className="flex items-center" variants={logoVariants} initial="initial" whileHover="hover">
            ...
            <nav className="hidden md:flex items-center space-x-1">
              {navigation.map((item) => (
                <motion.div key={item.name} variants={navItemVariants} initial="initial" whileHover="hover">
                  <Link
                    href={item.href}
            ...
            <div className="flex items-center space-x-2">
              ...
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button variant="ghost" size="sm" onClick={openWishlist} aria-label="Wishlist">
              ...
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button variant="ghost" size="sm" onClick={openCart} className="relative" aria-label={`Cart with ${cartItemCount} items`}>
              ...
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button variant="ghost" size="sm" onClick={() => setShowPerformanceDashboard(true)} aria-label="Performance Dashboard">
                  <BarChart3 className="w-4 h-4" />
                </Button>
              </motion.div>

              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button variant="ghost" size="sm" onClick={toggleMobileMenu} className="md:hidden" aria-label="Toggle menu">
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.nav className="md:hidden border-t bg-white/95 backdrop-blur-xl" variants={mobileMenuVariants} initial="closed" animate="open" exit="closed">
              <div className="px-4 py-4 space-y-2">
                ...
                <div className="pt-4 border-t space-y-2">
                  {[
                    ...
                    { icon: ShoppingCart, label: `Cart (${cartItemCount})`, onClick: openCart },
                    { icon: User, label: 'Account', onClick: undefined }
                  ].map((btn, idx) => (
                    <motion.div key={btn.label} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: (navigation.length + idx) * 0.1 }}>
                      <Button
                        variant="ghost"
                      ...
                      >
                        <btn.icon className="w-4 h-4 mr-3" />
                        {btn.label}
                      </Button>
                    </motion.div>
                  ))}
